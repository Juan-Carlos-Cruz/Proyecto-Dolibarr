...
### Ejecución integral (funcionales + unitarias + reportes)

Una vez instalado todo y con Dolibarr ejecutándose, defina la ruta local del core con la variable `DOLIBARR_ROOT` y ejecute:

```bash
DOLIBARR_ROOT=/ruta/a/dolibarr/htdocs bash scripts/run_full_suite.sh
```

El script exporta `BASE_URL`, `ADMIN_USER` y `ADMIN_PASS` (puede sobreescribirlos antes de invocar el comando) para lanzar Playwright con el reporter de Allure, corre PHPUnit con modo TestDox y finalmente genera el dashboard y el informe Word.
